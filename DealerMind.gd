...
func _highlight(card, on: bool):
	...
	if card.small:
		...
		base_scale = PODIUM_SCALE * card.podium_index
	if on:
		tween.tween_property(body, "scale", Vector2(base_scale + (0.4 if card.podium else 0.2), base_scale + (0.4 if card.podium else 0.2)), 0.4)
