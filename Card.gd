...
func podium_display_start():
	...
	tween.tween_property($Body, "scale", PODIUM_SCALE * 1.2, 0.6)
	tween.parallel().tween_property($Body, "global_position:y", screen_mid_y - 60, 0.6)
	...
	tween.set_ease(Tween.EASE_OUT)
	tween.tween_property($Body, "scale", PODIUM_SCALE * podium_index, 0.4)
	tween.parallel().tween_property($Body, "global_position:y", screen_mid_y - 3, 0.4)
